...
func registerCallbacks() {
	...
	// 使用Promise封装的HTTP调用函数
	js.Global().Set("sum_via_http", ToPromise(func(args []js.Value) interface{} {
		a := args[0].Int()
		b := args[1].Int()
		result, err := SumViaHTTP(a, b)
		if err != nil {
			// 抛出JS错误
			js.Global().Get("Error").New(err.Error()).Call("toString")
			return nil
		}
		return result
	}))
